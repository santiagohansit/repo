...
import type { McrFile, Image } from "@shared/schema";
...
export default function FileManagement() {
  ...
  const [editingFile, setEditingFile] = useState<McrFile | null>(null);
  const [keyboardExcludedKeys, setKeyboardExcludedKeys] = useState<string[]>([]);
  const [mcrCommands, setMcrCommands] = useState<McrCommand[]>([]);
  ...
  const { data: associatedImage, isLoading: isLoadingAssociatedImage } = useQuery<Image>({ 
    queryKey: ['/api/images', editingFile?.associatedMcrFileId],
    queryFn: async () => {
      if (!editingFile?.associatedMcrFileId) return null;
      const response = await apiRequest('GET', `/api/images/${editingFile.associatedMcrFileId}`);
      return response.json();
    },
    enabled: !!editingFile?.associatedMcrFileId,
  });

  const { data: parsedCommands, isLoading: isLoadingParsedCommands } = useQuery<McrCommand[]>({ 
    queryKey: ['/api/files', editingFile?.id, 'parsed-content'],
    queryFn: async () => {
      if (!editingFile?.id) return [];
      const response = await apiRequest('GET', `/api/files/${editingFile.id}/parsed-content`);
      return response.json();
    },
    enabled: !!editingFile?.id,
  });

  // Update mcrCommands when editingFile or parsedCommands change
  useEffect(() => {
    if (editingFile && parsedCommands) {
      setMcrCommands(parsedCommands);
    } else {
      setMcrCommands([]);
    }
  }, [editingFile, parsedCommands]);
  ...
  const updateExcludedKeysMutation = useMutation({
    mutationFn: async (variables: { fileId: string; excludedKeys: string[] }) => {
      return apiRequest('POST', `/api/files/${variables.fileId}/excluded-keys`, { excludedKeys: variables.excludedKeys });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/files'] });
      toast({
        title: "Excluded keys updated",
        description: "The excluded keys have been successfully updated.",
      });
  ...
  const handleAddCommand = (command: McrCommand) => {
    setMcrCommands(prev => [...prev, command]);
  };

  const updateMcrCommandsMutation = useMutation({
    mutationFn: async (variables: { fileId: string; commands: McrCommand[] }) => {
      return apiRequest('POST', `/api/files/${variables.fileId}/update-commands`, { commands: variables.commands });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/files'] });
      toast({
        title: "MCR commands updated",
        description: "The MCR commands have been successfully updated.",
      });
  ...
  return (
    <>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          title="Edit Keystrokes"
                          onClick={() => {
                            setEditingFile(file);
                            setIsKeyboardEditorOpen(true);
                          }}
                        >
                          <Keyboard className="w-4 h-4" />
                        </Button>
                        
                        {file.associatedMcrFileId && (
                          <Button
                            variant="ghost"
                            size="icon"
                            title="Image Preview"
                            onClick={() => {
                              setPreviewingImage(associatedImage);
      ...
      <Dialog open={isKeyboardEditorOpen} onOpenChange={setIsKeyboardEditorOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>Edit Excluded Keystrokes</DialogTitle>
            <DialogDescription>
              Select the keystrokes to exclude for the file "{editingFile?.originalName}".
            </DialogDescription>
          </DialogHeader>
          {isLoadingAssociatedImage || isLoadingParsedCommands ? (
            <div className="flex items-center justify-center h-32">
              <Loader2 className="w-6 h-6 animate-spin text-muted-foreground" />
              Loading associated image and commands...
            </div>
          ) : (
            <KeyboardEditor 
              onExcludedKeysChange={setKeyboardExcludedKeys} 
              associatedImage={associatedImage} 
              onAddCommand={handleAddCommand} 
              mcrCommands={mcrCommands} 
            />
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsKeyboardEditorOpen(false)}>Cancel</Button>
            <Button 
              onClick={() => {
                if (editingFile) {
                  updateMcrCommandsMutation.mutate({ fileId: editingFile.id, commands: mcrCommands });
                }
              }}
              disabled={updateMcrCommandsMutation.isPending}
            >
              {updateMcrCommandsMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Save MCR
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      {previewingImage && (
        <ImagePreviewDialog
          ...
          mcrCommands={mcrCommands} // Pass the current MCR commands
        />
      )}
    </>
  );
