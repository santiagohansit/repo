...
import { insertMcrFileSchema, humanizationSettingsSchema, insertImageSchema } from "@shared/schema";
...
const imageUpload = multer({
  dest: 'images/', // New directory for images
  fileFilter: (req, file, cb) => {
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/files/:id/duration", async (req, res) => {
    try {
      ...
      console.error("Error calculating duration for file:", req.params.id, error);
      if (error.code === 'ENOENT') {
        return res.status(404).json({ message: "MCR file not found on disk." });
      }
      res.status(500).json({ 
  ...
  // Update excluded keys for a file
  app.post("/api/files/:id/excluded-keys", async (req, res) => {
    try {
      const { excludedKeys } = req.body;
      if (!Array.isArray(excludedKeys)) {
        return res.status(400).json({ message: "Invalid excluded keys format" });
      }
      ...
      const currentSettings = file.humanizationSettings as any || {};
      const updatedSettings = { ...currentSettings, excludedKeys };

      await storage.updateMcrFile(req.params.id, { humanizationSettings: updatedSettings });

      res.json({ message: "Excluded keys updated successfully" });
    } catch (error) {
      res.status(500).json({ 
        message: error instanceof Error ? error.message : "Failed to update excluded keys" 
      });
    }
  });
  ...
  app.post("/api/images/upload", imageUpload.single('image'), async (req, res) => {
    try {
      ...
      if (!allowedMimeTypes.includes(req.file.mimetype)) {
        ...
        await fs.unlink(req.file.path).catch(() => {});
        return res.status(400).json({ message: "Only image files (jpeg, png, gif, bmp, webp) are allowed" });
      }
  ...
  // Associate MCR file with image
  app.post("/api/images/:id/associate-mcr", async (req, res) => {
    try {
      const { mcrFileId } = req.body;
      const imageId = req.params.id;

      if (!mcrFileId) {
        return res.status(400).json({ message: "MCR file ID is required" });
      }
      ...
      if (!mcrFile) {
        return res.status(404).json({ message: "MCR file not found" });
      }

      await storage.updateImage(imageId, { associatedMcrFileId: mcrFileId });

      res.json({ message: "MCR file associated successfully" });
    } catch (error) {
      res.status(500).json({ 
        message: error instanceof Error ? error.message : "Failed to associate MCR file with image" 
      });
    }
  });

  // Update MCR commands for a file
  app.post("/api/files/:id/update-commands", async (req, res) => {
    try {
      const { commands } = req.body;
      if (!Array.isArray(commands)) {
        return res.status(400).json({ message: "Invalid commands format" });
      }
      ...
      await storage.updateMcrFile(req.params.id, { originalCommands: commands.length });
      ...
      res.json({ message: "MCR commands updated successfully" });
    } catch (error) {
      res.status(500).json({ 
        message: error instanceof Error ? error.message : "Failed to update MCR commands" 
      });
    }
  });
